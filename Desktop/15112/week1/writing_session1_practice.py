...
def circlesIntersect(x1, y1, r1, x2, y2, r2):
    centerDistance = math.sqrt((x1-x2)**2+(y1-y2)**2)
    radiusDistance = r1+r2
...
def getInRange(x, bound1, bound2):
    ...
    returnValue = 0
    if x >= min(bound1,bound2) and x<= max(bound2,bound1):
        returnValue = x
    elif x <= bound1 and x <= bound2:
        returnValue = min(bound1,bound2)
    elif x >= bound2 and x >= bound1:
        returnValue = max(bound2,bound1)
    return returnValue
